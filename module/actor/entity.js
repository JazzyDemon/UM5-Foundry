...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("UM5E", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag('UM5E', 'transformOptions');
      const original = game.actors?.get(this.getFlag('UM5E', 'originalActor'));
      if (original) {
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("UM5E", "currencyWeight") ) {
    ...
    if ( this.getFlag("UM5E", "powerfulBuild") ) mod = Math.min(mod * 2, 8);
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("UM5E", "reliableTalent"));
    ...
    const rollData = mergeObject(options, {
      ...
      halflingLucky: this.getFlag("UM5E", "halflingLucky"),
  ...
  async rollDeathSave(options={}) {
    ...
    if ( this.getFlag("UM5E", "diamondSoul") ) {
      parts.push("@prof");
  ...
  async shortRest({dialog=true, chat=true, autoHD=false, autoHDThreshold=3}={}) {
    ...
    if ( chat ) {
      ...
      let restFlavor;
      switch (game.settings.get("UM5E", "restVariant")) {
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("UM5E", "allowPolymorphing");
